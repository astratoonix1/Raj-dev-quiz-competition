...
function setQuestionPool(questions, sourceLabel) {
  ...
  populateQuestionCountSelect(questions.length);
  el.startBtn.disabled = false;
  hideSetupError();
}

function populateQuestionCountSelect(total) {
  el.questionCountSelect.innerHTML = '';
  const options = [];
  for (let n = 1; n <= total; n++) options.push(n);
  // Keep the dropdown manageable for large banks: offer every value up to 10,
  // then sensible steps beyond that, always including the max.
  const stepped = options.filter(n => n <= 10 || n % 5 === 0 || n === total);
  stepped.forEach(n => {
    const opt = document.createElement('option');
    opt.value = n;
    opt.textContent = `${n} question${n === 1 ? '' : 's'}`;
    el.questionCountSelect.appendChild(opt);
  });
  const defaultCount = Math.min(10, total);
  el.questionCountSelect.value = String(defaultCount);
}
...
el.jsonUpload.addEventListener('change', async () => {
  ...
  try {
    ...
    setQuestionPool(questions, `"${file.name}"`);
  } catch (err) {
    showSetupError(`Couldn't load that file: ${err.message}`);
...
el.startBtn.addEventListener('click', () => {
  ...
  const count = Math.min(Number(el.questionCountSelect.value), state.allQuestions.length);
